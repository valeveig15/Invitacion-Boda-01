(() => {
  ...
  function updateMusicButton(){
    ...
    musicFab.title = playing ? 'Pausar música' : 'Reproducir música';
  }
  ...
  async function toggleMusic(){
    ...
    try{
      ...
      musicFab.setAttribute('aria-label','No se pudo reproducir. Tocá para reintentar');
      musicFab.title = 'No se pudo reproducir. Tocá para reintentar';
    }
  }

  musicFab?.addEventListener('click', toggleMusic);

  // Los navegadores bloquean el autoplay con sonido. Para que la música no
  // dependa exclusivamente de encontrar el botón, intentamos iniciarla en la
  // primera interacción real del visitante con la invitación.
  const startMusicOnFirstInteraction = async (event)=>{
    if(!audio || !audio.paused || musicOn) return;
    if(event.target?.closest?.('#musicFab')) return;
    try{
      audio.volume = .38;
      await audio.play();
      updateMusicButton();
    }catch(err){
      console.debug('El navegador mantuvo bloqueado el audio hasta usar el control musical.', err);
    }
  };
  document.addEventListener('pointerdown', startMusicOnFirstInteraction, {once:true, capture:true});
